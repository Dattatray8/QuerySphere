...
const Answer = () => {
    ...
    const { question, loading } = useQuestion(qId);
    ...
    const [aZoomImg, setAZoomImg] = useState<boolean>(false);
    ...
    const handleAddAnswer = async () => {
        try {
            console.log(answer);
            if (answer.length < 20) {
                Toast.show({ type: 'error', text1: "Answer is too short" })
                return
            }
            ...
            router.push(`/questions/q/${question?._id}`)
        } catch (error: any) {
    ...
    return (
        <View style={{ flex: 1 }}>
            {loading ? (
                <ActivityIndicator style={{ marginTop: 250 }} size={25} />
            ) : (
                <ScrollView keyboardShouldPersistTaps="handled">
                    ...
                    {frontendImage && mediaType === "image" && (
                        <Pressable onPress={() => setAZoomImg(true)}>
            ...
            {aZoomImg && mediaType === "image" && (
                <View style={styles.zoomImageContainer}>
                    <Pressable
                        style={styles.closeBtn}
                        onPress={() => setAZoomImg(false)}
                    >
